#!/usr/bin/env python2
#change by dongchangqing in 2021/12/14;dongchangqing revised in 2021/12/15;
#source in https://github.com/kunyao2015; 
...
#up2
        #print("[DEBUG]conv_params=",conv_params.conv_weight_dims)
...
#up0
        ...
        #print("[DEBUG]param_name=",param_name)
        #print("[DEBUG]channels_in=",channels_in)	#channels_in有误
        if param_category == 'bn':
            ...
            if suffix == 'weights':
                ...
                param_shape = [channels_out]
        #print("[DEBUG]param_shape=",param_shape)
        param_size = np.product(np.array(param_shape))
        #print("[DEBUG]param_size=",param_size)
...
class GraphBuilderONNX(object):
    ...
    def build_onnx_graph(
        ...
        for layer_name in layer_configs.keys():
            ...
            #print("[DEBUG]layer_name=",layer_name)
            #print("[DEBUG]layer_dict=",layer_dict)
            major_node_specs = self._make_onnx_node(layer_name, layer_dict)	#这里构建模型结构
            #print("[DEBUG]major_node_specs.name=",major_node_specs.name)
            #print("[DEBUG]before-self.major_node_specs=",self.major_node_specs)
            if major_node_specs.name is not None:
                self.major_node_specs.append(major_node_specs)
            #print("[DEBUG]self.major_node_specs=",self.major_node_specs)
        outputs = list()
        #print("*************************************")
...
#up3
        for layer_name in self.param_dict.keys():
            ...
            if layer_type == 'convolutional':
                #print("[DEBUG]params=",params.conv_weight_dims)	#这里发现构建的conv形状有误
    ...
    def _make_onnx_node(self, layer_name, layer_dict):
        ...
        """
        #print("[DEBUG_make_onnx_node]before-self.major_node_specs=",self.major_node_specs)
        ...
        if self.input_tensor is None:
            ...
            #print("[DEBUG_make_onnx_node]middle-self.major_node_specs=",self.major_node_specs)
            if layer_type in node_creators.keys():
                #print("[DEBUG]layer_type=",layer_type)
                #print("[DEBUG_make_onnx_node]**-self.major_node_specs=",self.major_node_specs)
                major_node_output_name, major_node_output_channels = \
                            node_creators[layer_type](layer_name, layer_dict)
                #print("[DEBUG]major_node_output_name=",major_node_output_name)
                #print("[DEBUG]major_node_output_channels=",major_node_output_channels)
                #print("[DEBUG_make_onnx_node]****-self.major_node_specs=",self.major_node_specs)
                major_node_specs = MajorNodeSpecs(major_node_output_name,
                                                  major_node_output_channels)
                #print("[DEBUG_make_onnx_node]af****-self.major_node_specs=",major_node_specs)
            else:
                print(
                    'Layer of type %s not supported, skipping ONNX node generation.' %
                    layer_type)
                major_node_specs = MajorNodeSpecs(layer_name,None)
        #print("[DEBUG_make_onnx_node]after-self.major_node_specs=",self.major_node_specs)
    ...
    def _make_conv_node(self, layer_name, layer_dict):
        ...
        layer_dict -- a layer parameter dictionary (one element of layer_configs)
        """
        #print("[DEBUG]========_make_conv_node=========")
        previous_node_specs = self._get_previous_node_specs()
        inputs = [previous_node_specs.name]
        #print("[DEBUG******]previous_NAME=",inputs)
        ...
        #print("[DEBUG******]previous_channels=",previous_channels)
        #print("[DEBUG******]weights_shape=",weights_shape)
    ...
    def _make_route_node(self, layer_name, layer_dict):
        ...
        #print("[DEBUG-ROUTE]after-self.major_node_specs=",self.major_node_specs)
        return layer_name, channels
...
def main():
    ...
    output_tensor_dims['064_convolutional'] = [24, 152, 152]
    output_tensor_dims['077_convolutional'] = [24, 76, 76]
    output_tensor_dims['090_convolutional'] = [24, 38, 38]
    ...
    weights_file_path = 'test.weights'
